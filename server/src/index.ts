...
const playerStats = new Map<string, { shots: number; maxCombo: number }>();
...
// Create a new player
function createPlayer(socketId: string, name: string, side: 'top' | 'bottom'): Player {
  const { TABLE_WIDTH, TABLE_HEIGHT, PADDLE_RADIUS } = GAME_CONFIG;
...
// Create a room
function createRoom(code: string | null = null): Room {
...
function startGameLoop(room: Room): void {
  ...
  room.gameLoop = setInterval(() => {
    ...
    const result = updatePhysics(room.gameState.puck, players, deltaTime);

    room.gameState.puck = result.puck;
    room.gameState.puckSpeed = pixelsToKmh(
      Math.sqrt(
        result.puck.velocity.x * result.puck.velocity.x +
...
// Handle a goal
function handleGoal(room: Room, scoredOn: 'top' | 'bottom'): void {
  const players = Array.from(room.players.values());
  ...
  if (scorer) {
    ...
    // Check for winner
    if (scorer.score >= GAME_CONFIG.WINNING_SCORE) {
      ...
      room.gameState.puck = resetPuck(scoredOn);
    }
  }
}
...
async function startCountdown(room: Room): Promise<void> {
  ...
  room.gameState.puck = createInitialPuck();
